...
public class LoadDatabase implements CommandLineRunner {
        ...
        public void run(String... args) throws Exception {
                ...
                for (ProjectSearchJson projectJson : projectJsons) {
                        ...
                        for (Versions version : versions) {
                                if (projectCreationDateTime == null
                                                || projectCreationDateTime.toLocalDate()
                                                                .isAfter(version.getStartDate())) {
                                        ZoneId zoneId = ZoneId.of(userJson.getTimeZone());
                                        projectCreationDateTime = version.getStartDate().atStartOfDay(zoneId)
                                                        .toOffsetDateTime();
                                }
                        }
                        ...
                        do {
                                        ...
                                        for (Issues singleIssue : listOfIssues) {
                                                ...
                                                issue.setIssueNumber(Integer.parseInt(
                                                                singleIssue.getKey().trim().substring(
                                                                                singleIssue.getKey().indexOf('-')
                                                                                                + 1)));
                                                String assignee = null;
                                                String assigneeUrl = null;
                                                if (singleIssue.getFields().getAssignee() != null) {
                                                        assignee = singleIssue.getFields().getAssignee()
                                                                        .getDisplayName();
                                                        assigneeUrl = singleIssue.getFields().getAssignee()
                                                                        .getAvatarUrls().getSize48();
                                                }
                                                issue.setAssignee(assignee);
                                                issue.setAssigneeAvatarUrl(assigneeUrl);
                                                issue.setCreatedDateTime(singleIssue.getFields().getCreated());
                                                if (projectCreationDateTime
                                                                .isAfter(singleIssue.getFields().getCreated())) {
                                                ...
                                                if (singleIssue.getFields().getDuedate() != null) {
                                                        ZoneId zoneId = ZoneId.of(singleIssue.getFields().getCreator()
                                                                        .getTimeZone());
                                                        dueDateTime = singleIssue.getFields().getDuedate()
                                                                        .atStartOfDay(zoneId)
                                                                        .toOffsetDateTime();
                                                }
                                                ...
                                                issue.setProjectName(
                                                                singleIssue.getFields().getProject().getName().trim());
                                                ...
                                                if (singleIssue.getFields().getResolution() != null) {
                                                        resolutionName = singleIssue.getFields().getResolution()
                                                                        .getName();
                                                }
                                                issue.setResolution(resolutionName);
                                                issue.setResolutionDateTime(
                                                                singleIssue.getFields().getResolutiondate());
                                ...
                                getIssues = Unirest.get(searchUrl).basicAuth(JiraUsername, JiraPassword)
                                                .header("Accept", "application/json").queryString("jql", jqlQuery)
                                                .asJson();
                ...
                System.out
                                .println("Please wait 30 minutes for the database to update! Current time is "
                                                + OffsetDateTime.now()
